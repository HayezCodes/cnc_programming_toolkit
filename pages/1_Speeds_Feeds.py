...
def render_center_drill_depth_block(section_key: str, pilot_diameter_in: float, unit_mode: str):
    st.markdown("### Center Drill Depth")

    body_default_in = max(pilot_diameter_in + 0.1250, pilot_diameter_in * 2)
    target_default_in = min(body_default_in, pilot_diameter_in + 0.0500)

    d1, d2, d3, d4 = st.columns(4)
    with d1:
        included_angle = st.number_input("Included Angle (deg)", min_value=1.0, max_value=179.0, value=60.0, step=1.0, format="%.1f", key=f"{section_key}_cd_angle")
    with d2:
        target_bell_diameter_in = diameter_input("Target Bell Diameter", f"{section_key}_cd_target_bell", unit_mode, target_default_in, 0.0010)
    with d3:
        body_diameter_in = diameter_input("Tool Body / Bell Diameter", f"{section_key}_cd_body", unit_mode, body_default_in, 0.0010)
    with d4:
        drill_length_in = diameter_input("Drill Length (C)", f"{section_key}_cd_length", unit_mode, 0.1250, 0.0010)
    ...
    r2.metric("Pilot Diameter", format_length(pilot_diameter_in, unit_mode))
    ...
    with lathe_tab2:
        ...
        with col3:
            if drill_type == "Center Drill":
                drill_diameter = diameter_input("Center Drill Pilot Diameter", "lathe_drill_dia", unit_mode, 0.1250, 0.0010)
            else:
        ...
        if drill_type == "Center Drill":
            render_center_drill_depth_block("lathe_drill", drill_diameter, unit_mode)
    ...
    with lathe_tab4:
        ...
        with col3:
            if drill_type_live == "Center Drill":
                drill_diameter_live = diameter_input("Center Drill Pilot Diameter", "lathe_live_drill_dia", unit_mode, 0.1250, 0.0010)
            else:
        ...
        if drill_type_live == "Center Drill":
            render_center_drill_depth_block("lathe_live_drill", drill_diameter_live, unit_mode)
...
with main_tab2:
    ...
    with col4:
        if tool_type == "Spot Drill":
            ...
            diameter = diameter_input("Center Drill Pilot Diameter", "center_drill_dia", unit_mode, 0.1250, 0.0010)
    ...
    if tool_type == "Spot Drill":
        ...
        render_center_drill_depth_block("mill_center_drill", diameter, unit_mode)
